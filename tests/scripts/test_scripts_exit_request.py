...
from scripts.kapi_client import Key, KapiKey
...
class TestBuildExitRequest:
    ...
    def test_build_exit_request_success(self):
        ...
        mock_kapi_client.get_keys.return_value = [
            KapiKey(module_id=1, no_id=10, validator_pub_key="0xaabbccdd", pub_key_index=5),
            KapiKey(module_id=2, no_id=20, validator_pub_key="0x11223344", pub_key_index=15),
        ]
    ...
    def test_build_exit_request_pubkey_not_found_in_kapi(self):
        ...
        mock_kapi_client = Mock()
        mock_kapi_client.get_keys.return_value = [
            KapiKey(module_id=1, no_id=10, validator_pub_key="0x99999999", pub_key_index=5),
        ]
        
        validator_indexes = [123]
        
        with pytest.raises(ValueError, match="Validator pubkey 0xaabbccdd not found in KAPI"):
            build_exit_request(mock_kapi_client, mock_cl_client, validator_indexes)
