...
class TestExitDataDecoding:
    ...
    def test_decode_exit_data_correctness(self):
        ...
        print(
            f"Packed request length: {PACKED_REQUEST_LENGTH_V1} bytes per validator (format 1)"
        )
    ...
    def test_decode_format2_with_key_index(self):
        ...
        result = unpack_exit_request(
            entry, 0, data_format=DATA_FORMAT_LIST_WITH_KEY_INDEX
        )
        ...
        assert (
            wrong["pubkey"] == corrupted_expected
    ...
    def test_decode_all_validators_format2(self):
        ...
        real_pubkey = bytes.fromhex(
            "ae2fd379751dc0256d7ea54eca4d14f8456aec60bcd55397f17f2f01fee04381f5ee7c388ef1bcf0".ljust(
                96, "0"
            )
        )
        metadata_int = (module_id << (64 + 40)) | (node_op_id << 64) | val_index
        ...
        validators = decode_all_validators(
            data, data_format=DATA_FORMAT_LIST_WITH_KEY_INDEX
        )
        assert len(validators) == 3
        for i, v in enumerate(validators):
            assert (
                v["pubkey"] == real_pubkey
            ), f"Validator {i} pubkey wrong: {v['pubkey'].hex()}"
