...
class ValidatorExitData:
    ...
    def __init__(self, exit_requests: list[Key]):
        ...
        self.exit_requests = sorted(
            exit_requests, key=lambda x: (x.module_id, x.no_id, x.validator_index)
        )
    ...
    def from_et_calldata(cls, calldata: HexStr) -> "ValidatorExitData":
        ...
        result = decode(
            ["(uint256,uint256,uint64,bytes,uint256)[]"], Web3.to_bytes(hexstr=calldata)
        )
        return result
    ...
    def to_veb_calldata(self) -> HexStr:
        ...
        for request in self.exit_requests:
            ...
            veb_calldata += request.module_id.to_bytes(
                self.MODULE_ID_LENGTH, byteorder="big"
            )

            # Node Operator ID (5 bytes) - matching original format
            veb_calldata += request.no_id.to_bytes(
                self.NODE_OPERATOR_ID_LENGTH, byteorder="big"
            )

            # Validator Index (8 bytes)
            veb_calldata += request.validator_index.to_bytes(
                self.VALIDATOR_INDEX_LENGTH, byteorder="big"
            )
